...
* Top row: cumulative reported cases (left) and deaths (right).  
* Middle row: cumulative reported cases (left) and deaths (right) per 10,000 population on a log10 scale.   
* Bottom row: weekly ratio of cumulative number of cases (left) and deaths (right) calculated for each of the past 7 days leading up to `r format(today, "%d/%m/%Y")`.
...
# add column signifying WR related to chosen country and remove countries with incomplete data and only data within last 8 days
 current_country_WR_cases <-  WHO_cases_and_deaths_weekly_ratio %>%
   ...
   mutate(current_country = (country == i)) %>% 
   group_by(country) %>% 
   filter(sum((ratio_c > 0 & is.finite(ratio_c))) == 8) %>%
   ungroup()
 ...
 current_country_WR_deaths <-  WHO_cases_and_deaths_weekly_ratio %>%
   ...
   mutate(current_country = (country == i)) %>% 
   group_by(country) %>% 
   filter(sum((ratio_d > 0 & is.finite(ratio_d))) == 8) %>%
   ungroup()
  ...
  seven_day_cum_cases <- ggplot() +
    ...
    geom_line(data = current_country_WR_cases %>% filter(current_country), aes(x=date,y=ratio_c,group=NULL),colour="black",size = 0.8) +
  ...
  seven_day_cum_deaths <- ggplot() +
    ...
    geom_line(data = current_country_WR_deaths %>% filter(current_country), aes(x=date,y=ratio_d,group=NULL),colour="black",size = 0.8) +
    labs(x="Date") +
    theme_classic(base_size = 15) +
    theme(axis.title.y = element_blank(), plot.margin = margin(0,15,0,0)) +
    scale_x_date(date_labels = "%b %d",breaks = c(today - 7, today - 5, today - 2, today)) +
